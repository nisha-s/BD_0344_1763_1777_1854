	...
	while (True):
		...
		while 1:
			...
			if data:
				...
				l1 = []           #creating a list containing details of the recieved job
				l1.append(res['job_id'])   
				l1.append(res['map_tasks'])  
				l1.append(res['reduce_tasks'])


				#writing logs(arrival time of the job)				
				...
				# l[0] is job-id , l[1] is a dictionary containing details of all map tasks of this job
				#l[2] is a dictionary containing details of all reduce tasks of this job

				

				#appending the list l to the job_recived(which contains details of all jobs , since this is shared variable among different threads, semaphore is used
				...
				#since the map tasks for the for the recieved job doesnt have any dependencies , it is pushed to the tasks queue,
				#since this is shared variable among different threads, semaphore is used
...
def fun2(l , algo):
	...
	if(algo == "LL"):
		while(True):
			if(len(tasks) == 0):    #checking if the task queue is empty
				continue
			else:
				#dispatch a task using the LEAST LOADED scheduling algorithm	
				max_slots = 0
				max_slots_port = 0
				for i in range(0,len(workers)):				#iterating the list of workers object to find the one with least loaded tasks
				...
				if(max_slots == 0):
					...
					while(semaphore2._value == 0):
						pass
					semaphore2.acquire()
					message = tasks.pop()		#popping the task from task queue 
					semaphore2.release()
					...
					s.send(message.encode())    #sending the task as a dictionary , to the worker
	...
	while (1):
		...
		while 1:
			...
			if data:
				...
				#print(res[0] , res[1] ,res[2]) #worker_id, free_slots, finished_task_id
									...
									for k in all_reduce_tasks:
										if(k['task_id'] == task_completed):
											while(semaphore1._value == 0):
												pass
											semaphore1.acquire()
											k['duration'] = 0  # update in the jobs_recieved list that this reduce task is done
											semaphore1.release()
											break;
									all_reducers_done = True   
									for k in all_reduce_tasks:    # check if all reducers are done
										if(k['duration'] != 0):
...
# config file and scheduling algo is taken as command line argument
